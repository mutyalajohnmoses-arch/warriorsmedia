...
export default function LiveStreamingSetupPage() {
  ...
  const startLocalPreview = async () => {
    ...
    try {
      ...
      if (programVideoRef.current) {
        ...
        try {
          await programVideoRef.current.play();
        } catch {}
      }
      ...
      if (err?.name === "NotAllowedError")
        msg = "Permission denied. Please grant access in your browser.";
  ...
  const { room, isConnected, connect, disconnect, toggleCameraTrack, toggleMicTrack } =
    useLiveKitRoom({
  ...
  if (!isClient) {
    return (
      <div className="p-6 text-center text-muted-foreground">
        Loading Broadcasting Studio Matrix...
      </div>
    );
  }
    ...
    <div className="p-6 max-w-7xl mx-auto space-y-6 text-foreground">
      ...
      <div className="flex justify-between items-center bg-card p-4 border rounded-xl shadow-sm">
        <div className="flex items-center space-x-3">
          <div
            className={`p-2 rounded-full ${isConnected ? "bg-red-500/20 text-red-600 animate-pulse" : "bg-muted text-muted-foreground"}`}
          >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="bg-card border rounded-xl p-4 flex flex-wrap gap-4 items-center justify-between">
            <div className="flex space-x-4">
              <div className="flex items-center space-x-1.5 text-xs">
                <span
                  className={`w-2 h-2 rounded-full ${cameraReady ? "bg-green-500" : "bg-rose-400"}`}
                />
                <span className="text-muted-foreground">
                  Camera Hardware: {cameraReady ? "Ready" : "Not Detected"}
                </span>
              </div>
              <div className="flex items-center space-x-1.5 text-xs">
                <span
                  className={`w-2 h-2 rounded-full ${micReady ? "bg-green-500" : "bg-rose-400"}`}
                />
                <span className="text-muted-foreground">
                  Audio Hardware: {micReady ? "Ready" : "Not Detected"}
                </span>
              </div>
            </div>
        ...
        <div className="space-y-6">
          <div className="bg-card border rounded-xl p-5 shadow-xs space-y-4">
            <h3 className="text-sm font-semibold tracking-wide uppercase text-muted-foreground border-b pb-2">
