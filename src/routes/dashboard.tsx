...
import { motion } from "framer-motion"; // Dynamic premium animations kosam add chesanu
import {
  Cross,
  LogOut,
  Radio,
...
function Home() {
  ...
  return (
    <main className="min-h-screen bg-[#020617] text-white selection:bg-blue-500/30 overflow-x-hidden relative font-sans antialiased">
      {/* Premium Ambient Faith Glow Layers */}
      ...
      <div className="max-w-7xl mx-auto px-6 py-10 space-y-12 relative z-10">
        ...
        <motion.section 
          ...
          className="relative overflow-hidden rounded-[24px] bg-[#0F172A]/40 backdrop-blur-xl border border-slate-800/80 p-8 flex flex-col md:flex-row md:items-center justify-between min-h-[180px] shadow-2xl"
        >
        ...
        <section className="space-y-4">
          ...
          {youtubeConnected && connectedChannel && (
            <motion.div 
              ...
              className="p-1 rounded-[24px] bg-gradient-to-r from-blue-500/20 to-purple-500/20"
            >
                ...
                <button
                  ...
                  className={`p-6 rounded-[24px] backdrop-blur-xl border transition-all duration-500 group relative overflow-hidden text-left ${
                    isDisabled
                      ...
                      : `bg-[#111827]/60 border-slate-800/60 hover:border-slate-700/80 ${module.borderGlow} cursor-pointer hover:-translate-y-1`
                  }`}
                >
                  {/* Subtle Background Inner Glow Gradient */}
                  {!isDisabled && (
                    <div className={`absolute inset-0 bg-gradient-to-br ${module.gradient} opacity-0 group-hover:opacity-100 transition-opacity duration-700 pointer-events-none`} />
                  )}
        ...
        {/* Downloader & Extractor Utility Tools */}
        <section className="space-y-4">
          <div className="flex items-center gap-2 border-b border-slate-800/60 pb-3">
        ...
        {/* Global Creator Guild & Team Roster */}
        <section className="space-y-4">
          <div className="flex items-center gap-2 border-b border-slate-800/60 pb-3">
                  ...
                  {profile && (
                    <div className="pt-4 border-t border-slate-800/60 grid grid-cols-2 gap-2 text-center font-mono">
                      <div className="bg-slate-950/50 p-2 rounded-xl border border-slate-900">
                        <span className="block text-xs font-bold text-white tracking-tight">{profile.followers}</span>
                        <span className="text-[9px] text-slate-500 uppercase tracking-wider">Followers</span>
                      </div>
                      <div className="bg-slate-950/50 p-2 rounded-xl border border-slate-900">
                        <span className="block text-xs font-bold text-white tracking-tight">{profile.posts}</span>
