...
function PrayerWall() {
  ...
  if (filter !== "all") {
    filteredPrayers = filteredPrayers.filter((p: any) => p.category === filter);
  }
  if (searchQuery) {
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="font-display text-2xl flex items-center gap-2">
              <HandHeart className="w-6 h-6 text-[color:var(--gold)]" />
              Prayer Wall
            </h1>
        ...
        <div className="grid grid-cols-3 gap-4">
          ...
          <div className="p-4 rounded-lg border border-border bg-card/50">
            ...
            <p className="text-2xl font-display mt-1 text-[color:var(--gold)]">
              {filteredPrayers.reduce((sum: number, p: any) => sum + p.agreed_count, 0)}
            </p>
          </div>
          <div className="p-4 rounded-lg border border-border bg-card/50">
            <p className="text-xs text-muted-foreground uppercase tracking-[0.2em]">
            ...
            <p className="text-2xl font-display mt-1">
              {filteredPrayers.filter((p: any) => p.is_own).length}
            </p>
          </div>
        </div>
      </section>
      ...
      <section className="px-6 md:px-10 max-w-5xl mx-auto">
        {prayersQuery.isLoading ? (
          ...
          <div className="text-center py-12">
            <HandHeart className="w-12 h-12 text-muted-foreground mx-auto mb-4 opacity-50" />
