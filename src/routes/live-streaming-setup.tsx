  ...
  return (
    <div className="min-h-screen bg-[#0f0f0f] text-[#f1f1f1] flex flex-col lg:flex-row p-6 gap-6">
      ...
      <div className="w-full lg:w-5/12 bg-[#1f1f1f] border border-[#2f2f2f] rounded-xl p-6 shadow-2xl flex flex-col gap-4">
        ...
        {/* --- BRAND NEW THUMBNAIL MODULE WITH FREE & PAID AI --- */}
            ...
            <div className="sm:col-span-7 flex flex-col gap-2">
              {thumbnailMode === "manual" ? (
                ...
                <div className="flex flex-col gap-1.5">
                  ...
                  {/* Dual AI Actions Grid */}
                  <div className="grid grid-cols-2 gap-2">
                    {/* Free AI Generation Action */}
                    <button
                      type="button"
                      disabled={isConnected || isGeneratingAI}
                      onClick={() => {
                        const promptToUse = aiPrompt.trim() || streamTitle.trim();
                        if (!promptToUse) {
                          toast.error("Please enter a Stream Title or Prompt for Free AI!");
                          return;
                        }
                        setIsGeneratingAI(true);
                        const tId = toast.loading("Generating Free AI Artwork...");
                        try {
                          const cleanPrompt = encodeURIComponent(promptToUse);
                          const seed = Math.floor(Math.random() * 90000) + 10000;
                          const freeUrl = `https://image.pollinations.ai/p/${cleanPrompt}?width=1280&height=720&seed=${seed}&nologo=true`;
                          setPreviewUrl(freeUrl);
                          toast.success("Free AI Thumbnail ready!", { id: tId });
                        } catch (err) {
                          toast.error("Free engine failed to respond.", { id: tId });
                        } finally {
                          setIsGeneratingAI(false);
                        }
                      }}
                      className="py-2 px-2 bg-emerald-600/10 border border-emerald-600/30 hover:bg-emerald-600/20 text-emerald-400 font-medium text-[11px] rounded-lg flex items-center justify-center gap-1 transition disabled:opacity-50"
                    >
                      <Sparkles className="w-3 h-3 text-emerald-400" />
                      <span>Free AI Create</span>
                    </button>

                    {/* Paid OpenAI Generation Action */}
                    <button
                      type="button"
                      onClick={handleGenerateAIThumbnail}
                      disabled={isConnected || isGeneratingAI}
                      className="py-2 px-2 bg-indigo-600/10 border border-indigo-600/30 hover:bg-indigo-600/20 text-indigo-300 font-medium text-[11px] rounded-lg flex items-center justify-center gap-1 transition disabled:opacity-50"
                    >
                      {isGeneratingAI ? (
                        <>
